...
class NewsAdapter(dataSet: List<News> = emptyList(),
                  val listener: (item : News, index : Int,bitmapFromView: Bitmap?)->Unit):
    DragDropSwipeAdapter<News, NewsAdapter.NewsViewHolder>(dataSet) {
    ...
    inner class NewsViewHolder(val binding: ItemNewsBinding) :
        ...
        init {
            ...
            binding.notFragView.setOnClickListener(this)
        }
    ...
    override fun getViewHolder(parent: View): NewsViewHolder {
        ...
        val binding = ItemNewsBinding.inflate(layoutInflater, parent as ViewGroup, false)
